Assumptions --------------------------------
	...
	3. County Codes used are FIPS (https://en.wikipedia.org/wiki/List_of_counties_in_Iowa)
...
Environment Setup and Starting the API ---------
	1. Download and Install Server LocalDB 
		- Installing SQL Server Express also works for this step (https://www.microsoft.com/en-us/sql-server/sql-server-downloads)
	
	2. Start SQL Server LocalDB (using Powershell)
		sqllocaldb start "MSSQLLocalDB" 
	
	3. Run Script to Create Tables and Entries
		** Must be ran in the root directory of the project
	...
	5. You can press the start button in the VS GUI or use Ctrl+F5 to start the API.
...
Database Cleanup and Stopping the Application ------------------------
	...
	3. Application can be stopped within the VS Code GUI.

Addressing Limited Details Within Swagger Doc ---------- 
	
Typically there would be more information on the swagger doc for to describe
what the function does, what the county codes are and possible responses from the API. 
There were some issues with getting the UI to generate a document including that information.
If you are interested, there are comments documenting details of the function more in depth in	
MainController.cs within the Controllers directory.
